...
export class UserNotificationResolver {
  ...
  @Subscription(() => NotificationSchema, {
    filter: (payload, vars, context) => {
      return payload?.notifyUser?.userId === context?.req?.extra?.user?.id;
    },
  })
  @SkipThrottle()
  notifyUser(@GqlUserId() userId: string) {
    return pubSub.asyncIterator('notifyUser');
  }
}
